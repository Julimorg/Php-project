...
if (strlen($_SESSION['user_id']) == 0) { 
    ...
    if (isset($_POST['update'])) { 
        $form_id = $_GET['form_id']; // Lấy ID đơn hàng từ tham số GET trong URL
        $status = $_POST['status']; // Lấy trạng thái mới từ form
        $remark = $_POST['remark']; // Lấy ghi chú từ form

        //? Thêm ghi chú vào bảng remark
        $query = mysqli_query($db, "insert into remark(frm_id, status, remark) values('$form_id', '$status', '$remark')");
        //? Update Status orders trong bảng users_orders
