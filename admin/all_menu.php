                            ...
                            <div class="card card-outline-primary">
                                ...
                                <div class="table-responsive m-t-40">
                                    <table id="example23" class="display nowrap table table-hover table-striped table-bordered" cellspacing="0" width="100%">
                                        ...
                                        <tbody>
                                            <?php
                                            $sql = "SELECT * FROM dishes order by d_id desc"; // Truy vấn lấy tất cả món ăn từ bảng dishes, sắp xếp giảm dần theo d_id
                                            $query = mysqli_query($db, $sql); // Thực thi truy vấn

                                            if (!mysqli_num_rows($query) > 0) { 
                                                echo '<td colspan="11"><center>No Menu</center></td>'; 
                                            } else {
                                                while ($rows = mysqli_fetch_array($query)) { // Lặp qua từng dòng kết quả từ bảng dishes
                                                    $mql = "select * from restaurant where rs_id='" . $rows['rs_id'] . "'"; // Truy vấn lấy thông tin nhà hàng dựa trên rs_id của món ăn
                                                    $newquery = mysqli_query($db, $mql); // Thực thi truy vấn
                                                    $fetch = mysqli_fetch_array($newquery); // Lấy dữ liệu nhà hàng dưới dạng mảng

                                                    //? Form HTML display menu info
                                                    echo '<tr><td>' . $fetch['title'] . '</td> <!-- Tên nhà hàng -->
                                                    <td>' . $rows['title'] . '</td> <!-- Tên món ăn -->
                                                    <td>' . $rows['slogan'] . '</td> <!-- Mô tả món ăn -->
                                                    <td>$' . $rows['price'] . '</td> <!-- Giá món ăn -->
                                                    <td><div class="col-md-3 col-lg-8 m-b-10">
                                                    <center><img src="Res_img/dishes/' . $rows['img'] . '" class="img-responsive radius" style="max-height:100px;max-width:150px;" /></center>
                                                    </div></td> <!-- Hiển thị hình ảnh món ăn -->
